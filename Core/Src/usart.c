 ...
 *
 * 环形缓冲区采用循环 FIFO 方式管理。
 * - usart1_rx_byte：保存中断接收的最新一字节。
 * - usart1_rx_buffer：USART1 接收数据的环形缓冲区。
 * - usart1_rx_head：环形缓冲区的下一个写入位置。
 * - usart1_rx_tail：环形缓冲区的下一个读取位置。
 * - usart1_rx_overflow：缓冲区满时丢弃数据并置位该标志。
 *
 * 性能统计累加器用于记录 RX/TX 的 CPU 周期、字节数和调用次数，
 * 便于计算和输出 CPU 占用情况。
 */
...
/*
 * 读取当前 DWT 周期计数器。
 * 该计数器随每个 CPU 时钟周期递增，用于 USART TX/RX 性能统计。
 */
static uint32_t USART1_GetCycleCount(void) { return DWT->CYCCNT; }

/*
 * 启用 DWT 周期计数器并重置计数值。
 * 必须先通过 CoreDebug 使能 DWT 单元，计数器才能运行。
 */
...
/*
 * 从 USART1 RX 环形缓冲区读取最多 max_len 字节到目标缓冲区中。
 * 返回从缓冲区中读取的字节数。
 */
 ...
 * USART1 用户回调与辅助 API。
 *
 * 本节包含驱动中断接收流程、回显接收数据、以及计算 CPU 使用率的
 * 用户自定义操作。
 */
...
/*
 * 通过 USART1 发送一个以 NUL 结尾的字符串。
 * 只有发送成功时，才将耗时累加到性能统计中。
 */
...
/*
 * USART1 的周期性 CPU 监测任务。
 * 在中断保护下复制统计计数，计算 CPU 使用率百分比，并通过 RTT 打印日志。
 */
